...
namespace Orleans
{
    internal class ClientGrainContext : IGrainContext, IGrainExtensionBinder, IGrainContextAccessor
    {
        ...
        public TTarget GetTarget<TTarget>() where TTarget : class => (TTarget)GetTarget(typeof(TTarget));
        public object GetTarget(Type targetType)
        {
            if (targetType.IsAssignableFrom(GetType())) return this;
            return default;
        }
