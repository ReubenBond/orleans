...
internal sealed class CallbackWorker(ILogger logger) : IThreadPoolWorkItem
{
    ...
    public async Task<int> GetRunningRequestCount(GrainInterfaceType grainInterfaceType)
    {
        ...
        Action countInterfaces = () =>
        {
            ...
            {
                if (item.Value.Message.InterfaceType == grainInterfaceType)
                {
                    ++count;
                }
            }
        ...
        _workItems.Enqueue(new(countInterfaces, WorkItemType.InvokeDelegate));
