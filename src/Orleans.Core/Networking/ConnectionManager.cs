        ...
        private class ConnectionEntry
        {
            ...
            public void ThrowIfRecentFailure(SiloAddress address, TimeSpan connectionRetryDelay)
            {
                ...
                if (lastFailure.HasValue)
                {
                    ...
                    var retryAfter = lastFailure.Value.Add(connectionRetryDelay);
                    var remainingDelay = retryAfter.Subtract(now);
                    if (remainingDelay > TimeSpan.Zero)
                    {
                        throw new ConnectionFailedException($"Unable to connect to {address}, will retry after {remainingDelay.TotalMilliseconds}ms");
                    }
                }
            }
        }
