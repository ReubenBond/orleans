    ...
    public async Task CleanupDefunctSiloEntriesAsync(DateTimeOffset beforeDate, CancellationToken cancellationToken = default)
    {
        ...
        while (await this._storage.ExecuteTransaction(async transaction =>
        {
            ...
            var defunct = snapshot.Documents
                ...
                .Take(FirestoreDataManager.MaxBatchSize - 1)
                .ToArray();
            if (defunct.Length == 0)
            {
                return false;
            ...
            var next = versionSnapshot.ConvertTo<ClusterVersionEntity>().ToTableVersion().Next();
            transaction.Update(versionSnapshot.Reference, CreateClusterVersionEntity(next.Version).GetFields(),
                Precondition.LastUpdated(versionSnapshot.UpdateTime!.Value));
