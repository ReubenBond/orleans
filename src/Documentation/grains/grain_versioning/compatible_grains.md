...
uid: grain_versioning_compatible_grains
---

# Compatible grains

When an existing grain activation is about to process a request, the runtime will check if the version in the request and the activated version of the grain are compatible.
__Orleans does not infer at runtime which policy to use__, instead the strategy used to determine if two versions are compatible is determined by `GrainVersioningOptions.CompatibilityStrategy`.

## Backward compatible (default)

### Definition

A grain interface version *Vn* is be backward compatible with *Vm* if:

* The name of the interface didn't change (or the overridden type code is identical)
* All public methods present in the Vm version are in the Vn version. __It is important that the signatures of the methods inherited from Vm are not modified__: since Orleans uses an internal built-in serializer, modifying/renaming a field (even private) can break serialization.
