...
internal static class FSharpUtilities
{
    ...
    public static bool IsUnionCase(LibraryTypes libraryTypes, INamedTypeSymbol symbol, [NotNullWhen(true)] out INamedTypeSymbol? sumType)
    {
        sumType = null;
    ...
    public class FSharpUnionCaseTypeDescription(Compilation compilation, INamedTypeSymbol type, LibraryTypes libraryTypes) : SerializableTypeDescription(compilation, type, false, GetUnionCaseDataMembers(libraryTypes, type), libraryTypes)
    {
        ...
        private class FSharpUnionCaseFieldDescription(LibraryTypes libraryTypes, IFieldSymbol field, uint ordinal) : IMemberDescription, ISerializableMember
        {
            private readonly LibraryTypes _libraryTypes = libraryTypes;
            private readonly IFieldSymbol _field = field;

            public uint FieldId { get; } = ordinal;
    ...
    public class FSharpRecordTypeDescription(Compilation compilation, INamedTypeSymbol type, LibraryTypes libraryTypes) : SerializableTypeDescription(compilation, type, false, GetRecordDataMembers(libraryTypes, type), libraryTypes)
    {
        ...
        private class FSharpRecordPropertyDescription(LibraryTypes libraryTypes, IPropertySymbol property, uint ordinal) : IMemberDescription, ISerializableMember
        {
            private readonly LibraryTypes _libraryTypes = libraryTypes;

            public uint FieldId { get; } = ordinal;

            public bool IsShallowCopyable => _libraryTypes.IsShallowCopyable(Type) || Property.HasAttribute(_libraryTypes.ImmutableAttribute);
            ...
            /// </summary>
            private IPropertySymbol Property { get; } = property;
