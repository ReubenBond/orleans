...
internal class SerializerGenerator(IGeneratorServices generatorServices)
{
        ...
        static MemberDeclarationSyntax GetFieldDeclaration(GeneratedFieldDescription description)
        {
            switch (description)
            {
                ...
                case FieldAccessorDescription accessor:
                    return GetFieldAccessorDeclaration(accessor);
                default:
    ...
    internal static MemberDeclarationSyntax GetFieldAccessorDeclaration(FieldAccessorDescription accessor)
    {
        if (accessor.InitializationSyntax is { } initialization)
        {
            return FieldDeclaration(VariableDeclaration(accessor.FieldType,
                    SingletonSeparatedList(VariableDeclarator(accessor.FieldName).WithInitializer(EqualsValueClause(initialization)))))
                .AddModifiers(Token(SyntaxKind.PrivateKeyword), Token(SyntaxKind.StaticKeyword), Token(SyntaxKind.ReadOnlyKeyword));
        }

        var receiver = Parameter(Identifier("instance")).WithType(accessor.ContainingType);
        if (accessor.ContainingTypeIsValueType)
        {
            receiver = receiver.AddModifiers(Token(SyntaxKind.RefKeyword));
        }

        var returnType = accessor.Kind switch
        {
            GeneratedAccessorKind.Field => RefType(accessor.FieldType),
            GeneratedAccessorKind.PropertyGetter => accessor.FieldType,
            _ => PredefinedType(Token(SyntaxKind.VoidKeyword)),
        };

        var parameters = accessor.Kind == GeneratedAccessorKind.PropertySetter
            ? SeparatedList([receiver, Parameter(Identifier("value")).WithType(accessor.FieldType)])
        ...
        List<ISerializableMember> members)
    {
        ...
        CodecFieldDescription GetCodecDescription(IMemberDescription member, int index)
        {
            ...
            else if (t is INamedTypeSymbol { ConstructedFrom: { } unboundFieldType } named && LibraryTypes.WellKnownCodecs.FindByUnderlyingType(unboundFieldType) is { } genericCodec)
            {
                ...
                codecType = genericCodec.CodecType.Construct([.. named.TypeArguments]).ToTypeSyntax();
            }
            else
            {
        ...
        ISerializableTypeDescription type)
    {
        ...
        Debug.Assert(staticCodec is not null);
    ...
    internal abstract class GeneratedFieldDescription(TypeSyntax fieldType, string fieldName)
    {
    ...
    internal sealed class ActivatorFieldDescription(TypeSyntax fieldType, string fieldName) : GeneratedFieldDescription(fieldType, fieldName)
    {
    ...
    {
        public TypeSyntax UnderlyingTypeSyntax { get; } = underlyingTypeSyntax;
        public ITypeSymbol UnderlyingType { get; } = underlyingType;
        public override bool IsInjected => false;
    }

    internal sealed class CodecFieldTypeFieldDescription(TypeSyntax fieldType, string fieldName, TypeSyntax codecFieldType) : GeneratedFieldDescription(fieldType, fieldName)
    {
        public TypeSyntax CodecFieldType { get; } = codecFieldType;
        public override bool IsInjected => false;
    }

    internal enum GeneratedAccessorKind : byte
    {
        Delegate,
        Field,
        PropertyGetter,
    ...
    internal sealed class FieldAccessorDescription(
        TypeSyntax containingType,
    ...
    /// </summary>
    internal class SerializableMethodMember(InvokableGenerator.MethodParameterFieldDescription member) : ISerializableMember
    {
        IMemberDescription ISerializableMember.Member => Member;
        public MethodParameterFieldDescription Member { get; } = member;

        private LibraryTypes LibraryTypes => Member.LibraryTypes;

        public bool IsShallowCopyable => LibraryTypes.IsShallowCopyable(Member.Parameter.Type) || Member.Parameter.HasAttribute(LibraryTypes.ImmutableAttribute);
    ...
    internal class SerializableMember(IGeneratorServices generatorServices, IMemberDescription member, int ordinal) : ISerializableMember
    {
        ...
        public bool IsShallowCopyable =>
            LibraryTypes.IsShallowCopyable(Member.Type)
            || Property is { } prop && prop.HasAttribute(LibraryTypes.ImmutableAttribute)
            || Member.Symbol.HasAttribute(LibraryTypes.ImmutableAttribute);

        public bool IsValueType => Type.IsValueType;

        public IMemberDescription Member { get; } = member;
        ...
        private bool CanUseUnsafeAccessor
        {
            get
            {
                if (LibraryTypes.UnsafeAccessorAttributeOrDefault is null)
                {
                    return false;
                }

                return IsPrimaryConstructorParameter && IsProperty
                    || !ContainingType.IsGenericType && !ContainsTypeParameter(Type);
            }
        }
        ...
        /// </summary>
        private IPropertySymbol? Property => field ??= field = Member.Symbol as IPropertySymbol ?? (Field is { } fieldSymbol ? PropertyUtility.GetMatchingProperty(fieldSymbol) : null);
        ...
        public ExpressionSyntax GetGetter(ExpressionSyntax instance)
        {
            ...
            if (Property is { } property && IsGettableProperty)
            {
                result = instance.Member(property.Name);
            }
            else if (Field is { } field && IsGettableField)
            {
        ...
        {
            // If the field is the backing field for an accessible auto-property use the property directly.
            if (Property is { } property && IsSettableProperty)
            {
            ...
            if (Field is { } field && IsSettableField)
            {
                return AssignmentExpression(
                    SyntaxKind.SimpleAssignmentExpression,
            ...
            if (UseUnsafeFieldAccessor)
            {
                return AssignmentExpression(
                    SyntaxKind.SimpleAssignmentExpression,
                    InvocationExpression(IdentifierName(SetterAccessorName))
                        .AddArgumentListArguments(instanceArg),
                    value);
            }

            return InvocationExpression(IdentifierName(SetterFieldName))
        ...
        public FieldAccessorDescription? GetSetterFieldDescription()
        {
            ...
            if (UseUnsafeFieldAccessor && !IsGettableField && !IsGettableProperty)
            {
                return null;
            }
