...
namespace Orleans.Clustering.DynamoDB
{
    internal partial class DynamoDBMembershipTable : IMembershipTable
    {
        ...
        private const int MAX_TRANSACTION_ITEMS = 100;
        ...
        public async Task<MembershipTableData> ReadRowAsync(SiloAddress siloAddress, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var entries = await storage.GetEntriesTxAsync(this.options.TableName,
                    new[] { siloEntryKeys, versionEntryKeys }, fields => new SiloInstanceRecord(fields), cancellationToken);
        ...
        public async Task<bool> UpdateRowAsync(MembershipEntry entry, string etag, TableVersion tableVersion, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    // Heartbeats do not change the logical row etag, so compare their value too
                    // when replacing the row to preserve the maximum observed heartbeat.
        ...
        public async Task CleanupDefunctSiloEntriesAsync(DateTimeOffset beforeDate, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    var defunct = records.Where(record => SiloIsDefunct(record, beforeDate))
                        .Take(MAX_TRANSACTION_ITEMS - 1)
                        .ToList();
                    if (defunct.Count == 0)
                    {
                        return;
                    ...
                    var current = records.SingleOrDefault(record => record.SiloIdentity == SiloInstanceRecord.TABLE_VERSION_ROW)
                        ?? throw new KeyNotFoundException("No version row found for membership table");
                    var next = new TableVersion(current.MembershipVersion, current.ETag.ToString(CultureInfo.InvariantCulture)).Next();
                    var version = new SiloInstanceRecord
                    {
                        DeploymentId = this.clusterId,
                        SiloIdentity = SiloInstanceRecord.TABLE_VERSION_ROW,
                        MembershipVersion = next.Version,
                        ETag = checked(current.ETag + 1)
                    };
                    var versionUpdate = new Update
                    {
                        TableName = this.options.TableName,
                        Key = version.GetKeys(),
                        ConditionExpression = $"{SiloInstanceRecord.ETAG_PROPERTY_NAME} = {CURRENT_ETAG_ALIAS}"
                    };
                    (versionUpdate.UpdateExpression, versionUpdate.ExpressionAttributeValues) = storage.ConvertUpdate(
                        version.GetFields(),
                        new() { [CURRENT_ETAG_ALIAS] = new AttributeValue { N = next.VersionEtag } });
                    var deletes = defunct.Select(record => new Delete
                    {
                        TableName = this.options.TableName,
                        Key = record.GetKeys(),
                        ConditionExpression = $"{SiloInstanceRecord.STATUS_PROPERTY_NAME} = :dead"
                            + $" AND {SiloInstanceRecord.ETAG_PROPERTY_NAME} = :etag"
                            + $" AND {SiloInstanceRecord.I_AM_ALIVE_TIME_PROPERTY_NAME} = :heartbeat",
                        ExpressionAttributeValues = new()
                        {
                            [":dead"] = new AttributeValue { N = ((int)SiloStatus.Dead).ToString(CultureInfo.InvariantCulture) },
                            [":etag"] = new AttributeValue { N = record.ETag.ToString(CultureInfo.InvariantCulture) },
                            [":heartbeat"] = new AttributeValue(record.IAmAliveTime)
                        }
                    });

                    try
                    {
                        await this.storage.WriteTxAsync(cancellationToken, updates: [versionUpdate], deletes: deletes);
                    }
                    catch (TransactionCanceledException exception) when (IsContention(exception))
                    {
                        // Re-select so concurrent heartbeats and status updates determine eligibility.
                    }
                }
            }
            catch (Exception exc)
        ...
        private static bool IsContention(TransactionCanceledException exception) =>
            exception.CancellationReasons?.Any(reason => reason.Code is "ConditionalCheckFailed" or "TransactionConflict") == true
            || exception.Message.Contains("ConditionalCheckFailed", StringComparison.Ordinal)
            || exception.Message.Contains("TransactionConflict", StringComparison.Ordinal);
