...
#pragma warning restore IDE1006 // Naming Styles
    ...
    private sealed class ArcBufferPool
    {
        public static readonly ArcBufferPool Shared = new();
        public const int MinimumBlockSize = 4 * 1024;
        private readonly ConcurrentQueue<ArcBufferPage> _blocks = new();
        private readonly ConcurrentQueue<ArcBufferPage> _largeBlocks = new();

        private ArcBufferPool() { }
    ...
    public sealed class ArcBufferPage : ReadOnlySequenceSegment<byte>
    {
        ...
        public void ResizeLargeSegment(int length)
        {
            Debug.Assert(length > ArcBufferPool.MinimumBlockSize);
            InitializeArray(length);
        }
        ...
        private void InitializeArray(int length)
        {
            if (length <= ArcBufferPool.MinimumBlockSize)
            {
                Debug.Assert(Array is null);
#if NET6_0_OR_GREATER
                var array = GC.AllocateUninitializedArray<byte>(ArcBufferPool.MinimumBlockSize, pinned: true);
#else
                var array = new byte[ArcBufferPool.MinimumBlockSize];
#endif
                Array = array;
            }
            else
        ...
        public bool IsValid => Array is { Length: > 0 };
        public bool IsMinimumSize => Array.Length == ArcBufferPool.MinimumBlockSize;
        ...
        public void SetNext(ArcBufferPage next) => Next = next;
        ...
        public void Unpin(int token)
        {
            ...
            if (Interlocked.Decrement(ref _refCount) == 0)
            {
                ...
                ArcBufferPool.Shared.Return(this);
            }
        }
    ...
    /// Initializes a new instance of the <see cref="BufferSlice"/> type.
    /// </summary>
    /// <param name="page">The first page in the buffer.</param>
    /// <param name="offset">The offset into the buffer page at which this slice begins.</param>
    /// <param name="length">The length of this slice.</param>
    public struct ArcBufferSlice(ArcBufferPage page, int offset, int length)
    {
        private readonly ArcBufferPage _first = page;
        private readonly int _offset = offset;
        private readonly int _length = length;
        private readonly int _token = page.Version;

        /// <summary>
        /// Gets the first <see cref="ArcBufferPage"/> represented by this instance.
        /// </summary>
        public readonly ArcBufferPage First => _first;
        ...
        */
        ...
        {
            var enumerator = new PageEnumerator(this);

            // Validate that the stored token matches the first page's token.
            // This can catch some use-after-free type bugs.
            if (enumerator.MoveNext())
            {
                enumerator.Current.Unpin(_token);
            }

            // Subsequently, use the page's current token.
        ...
        public readonly void Pin()
        {
            ...
            // Validate that the stored token matches the first page's token.
            // This is intended to catch some use-after-free type bugs.
            if (enumerator.MoveNext())
            {
                enumerator.Current.Pin(_token);
            }
        ...
        /// <summary>
        /// Enumerates over the pages in a <see cref="ArcBufferSlice"/>.
        /// </summary>
        private ref struct PageEnumerator
        {
            private static readonly ArcBufferPage InitialPageSentinel = new();
            private static readonly ArcBufferPage FinalPageSentinel = new();
            private readonly ArcBufferSlice _slice;
            private int _position;
            private ArcBufferPage _page;

            /// <summary>
            /// Initializes a new instance of the <see cref="PageEnumerator"/> type.
            /// </summary>
            /// <param name="slice">The slice to enumerate.</param>
            public PageEnumerator(ArcBufferSlice slice)
            {
                _slice = slice;
                _page = InitialPageSentinel;
                Current = null;
            }

            public PageEnumerator GetEnumerator() => this;

            internal readonly ArcBufferPage First => _slice._first;
            internal readonly int Offset => _slice._offset;
            internal readonly int Length => _slice._length;
            ...
            public bool MoveNext()
            {
                if (ReferenceEquals(_page, InitialPageSentinel))
                {
                    _page = First;
                }

                var endPosition = Offset + Length;
                while (_page != null && _page != FinalPageSentinel)
                {
                    var page = _page.CommittedMemory.Span;

                    // Find the starting page and the offset to copy from.
                    int pageOffset;
                    if (_position < Offset)
                    {
                        if (_position + page.Length <= Offset)
                        {
                            // Start is in a subsequent page
                            _position += page.Length;
                            _page = Unsafe.As<ArcBufferPage>(_page.Next);
                            continue;
                        }
                        else
                        {
                            // Start is in this page
                            pageOffset = Offset;
                        }
                    }
                    else
                    ...
                    var pageLength = Math.Min(page.Length - pageOffset, endPosition - (_position + pageOffset));
                    if (pageLength == 0)
                    {
                        Current = null;
                        _page = FinalPageSentinel;
                        return false;
                    }

                    Current = _page;
                    _position += pageOffset + pageLength;
                    _page = Unsafe.As<ArcBufferPage>(_page.Next);
                    return true;
                }
        ...
        public ref struct SpanEnumerator
        {
            ...
            /// </summary>
            /// <param name="slice">The slice to enumerate.</param>
            public SpanEnumerator(ArcBufferSlice slice)
            {
                _slice = slice;
                _page = InitialPageSentinel;
                Current = [];
            ...
            public bool MoveNext()
            {
                ...
                while (_page != null && _page != FinalPageSentinel)
                {
                    ...
                    Current = page.Slice(pageOffset, pageLength);
                    _position += pageOffset + pageLength;
                    _page = Unsafe.As<ArcBufferPage>(_page.Next);
                    return true;
                }
        ...
        public struct ArraySegmentEnumerator(ArcBufferSlice slice)
        {
            ...
            private int _position;
            private ArcBufferPage _page = InitialPageSentinel;
            ...
            public bool MoveNext()
            {
                ...
                while (_page != null && _page != FinalPageSentinel)
                {
                    var page = _page.CommittedArraySegment;

                    // Find the starting page and the offset to copy from.
                    int pageOffset;
                    if (_position < Offset)
                    {
                        if (_position + page.Count <= Offset)
                        {
                            // Start is in a subsequent page
                    ...
                    var pageLength = Math.Min(page.Count - pageOffset, endPosition - (_position + pageOffset));
                    if (pageLength == 0)
                    {
                        Current = ArraySegment<byte>.Empty;
                        _page = FinalPageSentinel;
