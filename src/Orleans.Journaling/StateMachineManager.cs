...
using Orleans.Journaling.Internal;
...
internal sealed class StateMachineManager : IStateMachineManager, ILifecycleParticipant<IGrainLifecycle>, ILifecycleObserver
{
    ...
    private readonly Dictionary<uint, IDurableStateMachine> _stateMachinesMap = new();
...
#pragma warning restore IDE0052 // Remove unread private members
    ...
    private sealed class StateMachineLogWriter : IStateMachineLogWriter
    {
        private readonly StateMachineManager _manager;
        private readonly StateMachineId _id;

        public StateMachineLogWriter(StateMachineManager manager, StateMachineId streamId)
        {
            _id = streamId;
            _manager = manager;
        }
    ...
    {
        public WorkItem(WorkItemType type, TaskCompletionSource? completion)
        {
            Type = type;
    ...
    private sealed class StateMachineManagerState : DurableDictionary<string, uint>
    {
        private readonly StateMachineManager _manager;

        public StateMachineManagerState(
            StateMachineManager manager,
            IFieldCodec<string> keyCodec,
            IFieldCodec<uint> valueCodec,
            SerializerSessionPool serializerSessionPool) : base(keyCodec, valueCodec, serializerSessionPool)
        {
            _manager = manager;
        }
