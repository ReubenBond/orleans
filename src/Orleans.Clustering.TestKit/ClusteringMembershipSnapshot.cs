...
{
    private readonly Dictionary<int, ClusteringMembershipSnapshot> _versions = [];
    ...
    {
        if (_versions.TryGetValue(observation.Version, out var prior))
        {
            ClusteringTestKitDiagnostics.Require(prior.CompareVersioned(observation) is null,
                $"same-version drift: {prior.CompareVersioned(observation)}");
        }
        else
        {
            _versions.Add(observation.Version, observation);
        }
