...
/// <remarks>Use a fresh initialized fixture for each method. No method repairs stale-table failures or infrastructure errors.</remarks>
public sealed class MembershipTableTestRunner
{
    ...
    /// <summary>G03: heartbeat preserves the complete versioned view; Dead compaction commits a new view.</summary>
    public Task Reads_SameVersion_HaveSameFullVersionedProjection(CancellationToken cancellationToken = default) => Run(async ct =>
    {
        await Seed(ct);
    ...
    /// <summary>G25: eligible Dead rows compact in atomic versioned batches; other rows and no-op cleanup remain unchanged.</summary>
        ...
        bool requireAllEligible = true)
    {
        ...
        Check(delta >= 1 && delta <= removed.Length,
            $"cleanup version: expected delta in [1,{removed.Length}], observed={delta}");
        Check(!string.IsNullOrEmpty(after.TableEtag) && after.TableEtag != before.TableEtag,
            $"cleanup table ETag must change: old={before.TableEtag}, observed={after.TableEtag}");
        EqualAllowingRefreshedRowEtags(before with
        {
    ...
    private async Task CleanupWithConcurrentReads(CancellationToken ct)
    {
        ...
        async Task Reader(int index)
        {
            ...
            for (var i = 0; i < 8; i++)
            {
                ...
                AssertCleanup(before, sample, T1, requireAllEligible: false);
        ...
        var history = new MembershipHistory();
        var previous = before;
        foreach (var observed in observations.OrderBy(snapshot => snapshot.Version))
        {
    ...
    private async Task ConcurrentReads(bool pointReads, CancellationToken ct)
    {
        ...
        for (var round = 0; round < 6; round++)
        {
            ...
            {
                var expected = sample.Version == before.Version ? before : committed;
                Equal(pointReads ? expected.Select(key) : expected, sample);
            }
        }
    }
}
