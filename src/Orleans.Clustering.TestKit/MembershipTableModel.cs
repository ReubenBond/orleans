...
internal static class MembershipModel
{
    ...
    internal static void Apply(MembershipRequest request, MembershipModelState next)
    {
...
internal sealed record MembershipModelResult(string? Failure, ClusteringMembershipSnapshot? Observation = null, int Seed = 0, int VersionOrigin = 0)
{
    ...
    internal async Task<MembershipModelResult> ExecuteAsync(MembershipRequest request)
    {
        ...
        try
        {
            ...
            else if (request.Kind == MembershipOperationKind.CleanupDead)
            {
                ...
                if (after.Version != before.Version) _previousTable = before.Next();
            }
            ...
            return new(null, after, _seed, _versionOrigin.Value);
        }
