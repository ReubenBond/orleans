...
{
    internal class SystemServiceGrainContext : IGrainContext, IGrainExtensionBinder, IAsyncDisposable, IGrainManagementExtension, IWorkItemScheduler
    {
        ...
        // int.MinValue is used as a sentinel value to indicate that the instance should not immediately invoke incoming requests
        // and must drain currently running requests.
        private int _runningRequestCount = int.MinValue;
...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        private int RunningRequestCount
        {
            get
            {
                var count = _runningRequestCount;
                if (count < 0)
                {
                    return count - int.MinValue;
                }
        ...
        private ActivationState State { get; set; }
        ...
        public void SetComponent<TComponent>(TComponent instance)
        {
            ...
            _components.TryAdd(typeof(TComponent), instance);
        }
        ...
        private string ToDetailedString(bool includeExtraDetails = false) =>
            ...
            + $"State={State} NumRunning={RunningRequestCount}";
        ...
        private async Task RunMessageLoop()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    {
                        if (_runningRequestCount >= 0)
                        {
                            // Begin draining running requests so that pending operations can be executed.
                            Interlocked.Add(ref _runningRequestCount, int.MinValue);
                        }

                        if (_runningRequestCount == int.MinValue)
                        {
                            // If running requests have been drained, execute pending operations
                            var dequeuedCount = 0;
                            try
                            {
        ...
        {
            if (!TryRecordRunningRequest())
            {
                // The activation is not able to process a request right now, so enqueue it instead.
                // Later on, it may be either invoked, rejected, or forwarded to a new activation.
        ...
        private bool TryRecordRunningRequest()
        {
            if (_runningRequestCount >= 0)
            {
                if (Interlocked.Increment(ref _runningRequestCount) > 0)
                {
                    return true;
                }
                else
        ...
        {
            var count = Interlocked.Decrement(ref _runningRequestCount);
        ...
        private void ProcessRequestsToInvalidActivation()
        {
            ...
            if (State is ActivationState.Deactivating)
            {
                ...
                {
                    // Do nothing until the activation completes deactivation or becomes stuck
                    return;
                }
            }
...
#region Deactivation
        ...
        public void QueueAction(Action<object> action, object state) => ThreadPool.UnsafeQueueUserWorkItem(action, state, preferLocal: true);
    }
}
