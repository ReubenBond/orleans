...
namespace Orleans.Worker.Gateway.Core;

public readonly struct GatewayMessage
{
    private readonly Message _message;
    internal GatewayMessage(Message message) => _message = message;

    public long CorrelationId => _message.Id.ToInt64();
    public GrainId SendingGrain => _message.SendingGrain;
    public GrainId TargetGrain => _message.TargetGrain;
    public SiloAddress SendingSilo => _message.SendingSilo;
    public SiloAddress TargetSilo => _message.TargetSilo;
    public Dictionary<string, object> RequestContextData => _message.RequestContextData;
    public Direction Direction => (Direction)_message.Direction;
    public ResponseType Result => (ResponseType)_message.Result;

    public object GetBody() => _message.BodyObject;

    // Request properties
    public TimeSpan? TimeToLive => _message.TimeToLive;
    public GrainInterfaceType InterfaceType => _message.InterfaceType;
    public ushort InterfaceVersion => _message.InterfaceVersion;

    // Response properties
    public RejectionResponse? GetRejectionResponse() => Result is ResponseType.Rejection ? _message.BodyObject as RejectionResponse : null;
}

public sealed class RejectionResponse
{
    public required string Message { get; init; }
    public RejectionType Type { get; init; }
...
public enum RejectionType : byte
{
    None,
    Transient,
    Overloaded,
...
/// <summary>
/// Provides functionality for gateways serving external clients and worker processes.
/// </summary>
public interface IGatewayRuntime
{
    // Client management
    GrainId RegisterClient(/* ??? */);
    void UnregisterClient(GrainId clientId);
    void PublishClientManifest(GrainId clientId, GrainManifest clientManifest);

    void SendRequest(ref WorkerRequest request);

    void SendResponse(long requestId, Response response);

}

public interface IWorkerManager
{
    void RegisterWorker(WorkerConnection connection);
    void UnregisterWorker(WorkerConnection connection);
}

public interface IWorkerRuntime
{
    IWorkerManager WorkerManager { get; }
    void SendMessage(WorkerConnection worker, GatewayMessage message);
}

public abstract class WorkerConnection : IDisposable
{
    private IWorkerRuntime? _runtime;

    public void OnRegistered(IWorkerRuntime runtime)
    {
        _runtime = runtime;
        OnRegisteredCore(runtime);
    }

    public Guid Id { get; } = Guid.NewGuid();

    public virtual void OnRegisteredCore(IWorkerRuntime runtime)
    {
    }

    private IWorkerRuntime Runtime => _runtime ??  throw new InvalidOperationException("Worker has not been registered with a runtime.");

    internal void OnMessageReceived(GatewayMessage message) => OnMessageReceivedCore(message);

    protected abstract void OnMessageReceivedCore(GatewayMessage message);

    protected void SendMessage(GatewayMessage message) => Runtime.SendMessage(this, message);

    public virtual void Dispose()
    {
        GC.SuppressFinalize(this);
        _runtime?.WorkerManager.UnregisterWorker(this);
    }
}

internal class WorkerGrainContext : IGrainContext, IAsyncDisposable, IActivationLifecycleObserver
{
