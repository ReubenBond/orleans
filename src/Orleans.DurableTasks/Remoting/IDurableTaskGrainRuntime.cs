...
{
    ValueTask<DurableTaskExecutionContext> EvaluateStepAsync(TaskId taskId, DurableTask taskDefinition, CancellationToken cancellationToken);
}
...
*/
    ...
    public async ValueTask<DurableTaskExecutionContext> EvaluateStepAsync(TaskId taskId, DurableTask durableTask, CancellationToken cancellationToken)
    {
        if (_shared.Logger.IsEnabled(LogLevel.Trace))
        {
    ...
    public async IAsyncEnumerable<(TaskId TaskId, DurableTaskDiagnosticState State)> GetTasksAsync()
    {
        ...
        foreach (var task in _pendingTasks.ToList())
        {
            ...
            var state = new DurableTaskDiagnosticState
            {
                ...
                Waiters = taskState.Observers?.Select(static client => client.ToString()!).ToList() ?? new List<string>(),
            };
