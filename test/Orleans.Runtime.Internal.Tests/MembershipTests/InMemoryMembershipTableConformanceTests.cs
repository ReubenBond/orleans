...
public partial class InMemoryMembershipTableTests
{
    ...
    public void CleanupDefunctSiloEntries_RecentDeathVoteProtectsStaleHeartbeat()
    {
        ...
        Assert.Equal(before.Version.Version + 1, removed.Version.Version);
        Assert.NotEqual(before.Version.VersionEtag, removed.Version.VersionEtag);
    ...
    [InlineData(false)]
    [InlineData(true)]
    public void CleanupDefunctSiloEntries_OneBatchOrOverflowPreservesAtomicity(bool atVersionLimit)
    {
        ...
        if (atVersionLimit)
        {
            Assert.Throws<OverflowException>(() => table.CleanupDefunctSiloEntries(cutoff));
            var after = table.ReadAll();
            Assert.Equal(before.Version, after.Version);
