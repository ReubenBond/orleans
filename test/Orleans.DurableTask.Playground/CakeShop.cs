...
public class CakeShopGrain : DurableGrain, ICakeShopGrain
{
    public async Task Run()
    {
        ...
        var orderTask = await orderProcessor.ProcessOrderAsync(buyer, order).ScheduleAsync($"order-66-{Guid.NewGuid()}");
...
public class OrderProcessorGrain : DurableGrain, IOrderProcessor
{
    ...
    {
        _status.Value = OrderStatus.Created;
        ...
        _status.Value = OrderStatus.Shipped;
    ...
    {
        var delayTask = DurableTask.Delay(TimeSpan.FromSeconds(5)).AsStep("wait-for-confirmation").AsTask();
...
public static class CakeShop
{
    ...
    public static async Task Run(IGrainFactory grainFactory) => await grainFactory.GetGrain<ICakeShopGrain>(0).Run();
}
...
{
    public DurableTask<List<StockCheckResult>> CheckOrderStock(Order order) => DurableTask.Run(async () =>
    {
        await Task.Delay(1200);
        return new List<StockCheckResult> { new StockCheckResult(true) };
    });
}

public class LogisticsService : ILogisticsService
{
    public DurableTask<CreateShipmentResult> CreateShipment(Order order) => DurableTask.Run(async () =>
    {
        await Task.Delay(1200);
        return new CreateShipmentResult(true);
    });

    public DurableTask WaitForDelivery(CreateShipmentResult shipmentDetails) => DurableTask.Delay(TimeSpan.FromSeconds(3));
}
