    ...
    public static async Task Main(string[] args)
    {
        var hostBuilder = Host.CreateDefaultBuilder(args)
            .UseOrleans(siloBuilder =>
            {
                ...
                {
                    optionsBuilder.Configure((AzureAppendBlobStateMachineStorageOptions options, ILoggerFactory loggerFactory) => {
                        options.ConfigureBlobServiceClient(async ct =>
                        {
                            var client = new BlobServiceClient("DefaultEndpointsProtocol=https;AccountName=rbnstor;AccountKey=YLMI6gPvI3XTKFcSvJ7GvEfzwUYMqEHlM0MTdhOcLtc9X6Mzp9J4kTCnWTp9bEh0BnpB363grEVw+AStc6WG5w==;EndpointSuffix=core.windows.net");
                            return client;
                        });
                    });
                });
...
{
    private readonly ILogger<AzureAppendBlobLogStorage> _logger = loggerFactory.CreateLogger<AzureAppendBlobLogStorage>();
