...
namespace UnitTests.Serialization
{
    ...
    public class MessageSerializerTests
    {
        ...
        private class MessageSerializerBackwardsCompatibilityStub(IFieldCodec<GrainAddress> grainAddressCodec)
        {
            internal List<GrainAddress> ReadCacheInvalidationHeaders<TInput>(ref Reader<TInput> reader)
            {
                ...
                if (n > 0)
                {
                    ...
                    for (int i = 0; i < n; i++)
                    {
                        list.Add(grainAddressCodec.ReadValue(ref reader, reader.ReadFieldHeader()));
                    }
            ...
            internal void WriteCacheInvalidationHeaders<TBufferWriter>(ref Writer<TBufferWriter> writer, List<GrainAddress> value) where TBufferWriter : IBufferWriter<byte>
            {
                ...
                {
                    grainAddressCodec.WriteField(ref writer, 0, typeof(GrainAddress), entry);
                }
            }
        }
    }
