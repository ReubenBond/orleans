...
namespace Orleans.Serialization.UnitTests
{
    ...
    public class PooledBufferTests
    {
        ...
        public void LargeBufferRoundTrip()
        {
            ...
            var memoryArray = new byte[randomData.Length];
            var memoryArrayMemory = memoryArray.AsMemory();
            foreach (var memory in buffer.AsMemorySequence())
            {
                memory.CopyTo(memoryArrayMemory);
                memoryArrayMemory = memoryArrayMemory.Slice(memory.Length);
            }
            Assert.Equal(randomData, memoryArray);
