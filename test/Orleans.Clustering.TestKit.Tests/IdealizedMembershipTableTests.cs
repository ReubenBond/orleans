...
public sealed class IdealizedMembershipTableTests
{
    ...
    [Fact]
    public async Task Cleanup_UsesDeadOnlyAndStrictEffectiveTimeCutoff()
    {
        var table = new IdealizedMembershipBackend().Create("A");
        ...
        Assert.Equal(repeated.Version.Version + 1, boundaryRemoved.Version.Version);
        Assert.NotEqual(repeated.Version.VersionEtag, boundaryRemoved.Version.VersionEtag);
