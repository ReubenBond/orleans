...
{
    [Fact]
    public void TableVersion_NextPreservesTokenAndRejectsOverflow()
    {
        var previous = new TableVersion(int.MaxValue - 1, "opaque-version-token");
        var last = previous.Next();
        Assert.Equal(int.MaxValue, last.Version);
        Assert.Equal(previous.VersionEtag, last.VersionEtag);
        Assert.Throws<OverflowException>(() => last.Next());
        Assert.Equal(int.MaxValue, last.Version);
    }
