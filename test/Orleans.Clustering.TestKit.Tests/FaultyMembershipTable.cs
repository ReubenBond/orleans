...
{
    IgnoreTableToken, IgnoreRowToken, FalseWriteChangesHeartbeat, VersionJump, HeartbeatRegression,
    ...
    SameVersionDeadRemoval, CleanupRoundsExclusiveCutoff, TornCleanupReadAll, TornCleanupReadRow
}
...
internal sealed class FaultyMembershipTable(MembershipFaultController control, string cluster, IMembershipTable inner) : IMembershipTable
{
    ...
    public async Task CleanupDefunctSiloEntriesAsync(DateTimeOffset beforeDate, CancellationToken cancellationToken = default)
    {
        ...
        var before = Fault == MembershipFault.SameVersionDeadRemoval ? await inner.ReadAllAsync(cancellationToken) : null;
        ...
        if (before is not null)
            Mutate(partition =>
            {
                ...
                {
                    partition.Version = before.Version.Version;
    ...
    public async Task UpdateIAmAliveAsync(MembershipEntry entry, CancellationToken cancellationToken = default)
    {
        ...
        if (Fault == MembershipFault.HeartbeatResurrectsCompactedRow && control.CleanupCompleted)
        {
            Mutate(partition =>
            {
                if (!partition.Rows.ContainsKey(entry.SiloAddress))
                {
                    partition.Rows.Add(entry.SiloAddress, Tuple.Create(IdealizedMembershipTable.Clone(entry), control.Backend.Token()));
                    control.Injected++;
                }
            });
        }
        if (Fault == MembershipFault.HeartbeatRegression)
            Mutate(p => { p.Rows[entry.SiloAddress].Item1.IAmAliveTime = entry.IAmAliveTime; control.Injected++; });
