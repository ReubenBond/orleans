...
public sealed class InProcessMembershipTableTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task CleanupDefunctSiloEntries_OneBatchOrOverflowPreservesAtomicity(bool atVersionLimit)
    {
        var entries = new[]
        {
        ...
        foreach (var entry in entries)
        {
            ...
            // Seed the boundary directly so overflow is exercised without billions of writes.
        ...
        if (atVersionLimit)
        {
            await Assert.ThrowsAsync<OverflowException>(() => _table.CleanupDefunctSiloEntriesAsync(cutoff, _cancellationToken));
            var after = await _table.ReadAllAsync(_cancellationToken);
            Assert.Equal(before.Version, after.Version);
            Assert.Equal(
                before.Members.Select(row => (row.Item1.ToFullString(), row.Item2)),
                after.Members.Select(row => (row.Item1.ToFullString(), row.Item2)));
        }
        else
        {
