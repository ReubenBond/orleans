...
public class StateMachineManagerTests : TestBase
{
    ...
    public async Task StateMachineManager_MultipleWriteStates_Test()
    {
        ...
        Assert.Equal(1, recoveredDict.Count);
    ...
    private class DurableValue<T>
    {
        ...
        public DurableValue(string name, StateMachineManager manager, IFieldCodec<T> codec, SerializerSessionPool sessionPool)
        {
            _name = name;
            _manager = manager;
            _codec = codec;
            ...
            stateMachine.OnUpdate += UpdateValue;
        }
        ...
        public T Value
        {
            ...
            set
            {
                ...
                var writer = session.CreateBufferWriter();
                _codec.WriteField(ref writer, 0, typeof(T), _value);
                var buffer = writer.ToByteArray();
                
                var stateMachine = _manager.GetStateMachine(_name);
                stateMachine.Write(buffer);
            }
        }

        private void RecoverValue(ReadOnlySpan<byte> buffer)
        {
            if (buffer.Length == 0)
            {
                return;
            ...
            _value = _codec.ReadValue(ref reader, 0);
            _initialized = true;
        }

        private void UpdateValue(ReadOnlySpan<byte> buffer)
        {
            RecoverValue(buffer);
        }

        private class SimpleStateMachine : IDurableStateMachine
        {
            private readonly StateMachineId _id;
            private byte[] _currentState = Array.Empty<byte>();

            public SimpleStateMachine(string name)
            {
                _id = new StateMachineId(name.GetHashCode());
                Name = name;
            }

            public event Action<ReadOnlySpan<byte>>? OnRecovery;
            public event Action<ReadOnlySpan<byte>>? OnUpdate;

            public string Name { get; }
            public StateMachineId Id => _id;

            public void Write(byte[] buffer)
            {
                _currentState = buffer;
            }

            public void OnStateRecovered(ReadOnlySpan<byte> state)
            {
                _currentState = state.ToArray();
                OnRecovery?.Invoke(state);
            }

            public void OnStateUpdated(ReadOnlySpan<byte> state)
            {
                _currentState = state.ToArray();
                OnUpdate?.Invoke(state);
            }

            public void WriteState(ref BufferWriter writer)
            {
