...
[TestCategory("GrainDirectory"), TestCategory("Functional"), TestCategory("GoogleFirestore"), TestCategory("GoogleCloud")]
public class FirestoreGrainDirectoryTests : GrainDirectoryTests<GoogleFirestoreGrainDirectory>
{
    public FirestoreGrainDirectoryTests(ITestOutputHelper testOutput) : base(testOutput)
    {
    }

    protected override GoogleFirestoreGrainDirectory GetGrainDirectory()
    {
        ...
        var loggerFactory = TestingUtils.CreateDefaultLoggerFactory("FirestoreGrainDirectoryTests.log");

        var directory = new GoogleFirestoreGrainDirectory(Options.Create(clusterOptions), Options.Create(options), loggerFactory);
        directory.Init().GetAwaiter().GetResult();
