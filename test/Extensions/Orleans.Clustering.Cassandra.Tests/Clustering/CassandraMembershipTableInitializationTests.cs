...
public sealed class CassandraMembershipTableInitializationTests
{
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Initialize_ConfiguredTtl_RejectsBeforeCreatingSession(bool gateway)
    {
        var factoryCalls = 0;
        var options = new CassandraClusteringOptions();
        Assert.False(options.UseCassandraTtl);
        options.UseCassandraTtl = true;
        options.ConfigureClient(_ =>
        {
            factoryCalls++;
            throw new InvalidOperationException("The session factory must not be called.");
        });
        ...
        var exception = await Assert.ThrowsAsync<OrleansConfigurationException>(() => gateway
            ? gatewayProvider.InitializeGatewayListProvider()
